...
class WrapperConfig @Inject() (appConfig: AppConfig)(implicit val messages: MessagesApi) {
  ...
  def ptaMinMenuConfig(implicit lang: Lang): PtaMinMenuConfig =
    PtaMinMenuConfig(menuName = messages("menu.name"), backName = messages("menu.back"))
  ...
  def menuConfig()(implicit request: AuthenticatedRequest[AnyContent], lang: Lang): Seq[MenuItemConfig] =
    btaConfig(
      Seq(
        ...
        MenuItemConfig(
          "progress",
          messages("menu.progress"),
          s"${appConfig.trackingHost}/track",
          leftAligned = false,
          position = 1,
          None,
          None
  ...
  private def btaConfig(config: Seq[MenuItemConfig])(implicit request: AuthenticatedRequest[AnyContent], lang: Lang) = {
    val showBta = request.enrolments
      .find(_.key == "IR-SA")
      .collectFirst { case Enrolment("IR-SA", Seq(identifier), "Activated", _) =>
        identifier.value
      }
      .isDefined
    ...
    val btaConfig = Seq(
      MenuItemConfig(
        "bta",
        messages("menu.bta"),
        s"${appConfig.businessTaxAccountUrl}",
        leftAligned = false,
        position = 3,
        None,
        None
